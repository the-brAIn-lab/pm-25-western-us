...
structure within each batch. This follows Patel et al.'s "time-split" batching
which performed best in their experiments — the model sees all sites for a
given set of days, learning spatial patterns with temporal variation controlled.

Usage: python loso_cv_daily_parallel.py [--target_batch 4000] [--n_epochs 200]
"""
import sys
sys.path.insert(0, '../../..')
import os
...
def run_fold(args):
    """Run a single LOSO fold with daily batch SGD."""
    (fold_idx, held_out_site, X_train, y_train_raw, X_test, y_test_raw,
     train_date_ordinals,
     base_indices, aot_idx, smogI_idx, smogP_idx, doy_idx,
     n_days, n_epochs, gpu_id, patience, inference_days) = args
    ...
    # Set conditioning data: sample complete days for inference
...
def main():
    ...
    parser.add_argument('--inference_days', type=int, default=200,
                        help='Number of days to use for inference conditioning')
    args = parser.parse_args()

    print(f"Configuration: n_gpus={args.n_gpus}, n_days={args.n_days}, "
          f"n_epochs={args.n_epochs}, patience={args.patience}, "
          f"inference_days={args.inference_days}")
    print(f"Batching strategy: DAILY ({args.n_days} day(s) per epoch)")
    ...
    for i, held_out_site in enumerate(sites):
        ...
        fold_args.append((
            ...
            train_date_ordinals,
            base_indices, aot_idx, smogI_idx, smogP_idx, doy_idx,
            args.n_days, args.n_epochs, gpu_id, args.patience, args.inference_days
        ))
    ...
    with ProcessPoolExecutor(max_workers=n_gpus) as executor:
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                nd = result['timing']['n_days_per_batch']
                ab = result['timing']['avg_batch_size']
                print(f"  Fold {result['fold_idx']:2d} (GPU {gpu}) site={site}: "
                      f"R²={r2:.3f}, epochs={ep}, days/batch={nd}, avg_batch={ab:.0f}, "
    ...
    print(f"\n{'='*60}")
    print(f"LOSO CV Results (Daily Batch SGD, Seasonal Interaction, {n_gpus} GPUs)")
    ...
    output = {
        'config': {
            ...
            'patience': args.patience, 'inference_days': args.inference_days,
    ...
    print("\nResults saved to daily_parallel_results.json, etc.")
